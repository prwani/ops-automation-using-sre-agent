...
The demo environment uses [Azure Jumpstart ArcBox for IT Pros](https://jumpstart.azure.com/azure_jumpstart_arcbox/ITPro) — a self-contained sandbox that deploys an entire simulated on-prem datacenter inside a single Azure subscription. All core components run real — no mocks, no fake data.
...
| PowerShell Scripts | Run from ArcBox-Client or Cloud Shell | ✅ Real | Same (or scheduled via Task Scheduler / cron) |
...
Step 3: Deploy Solution Stack
  ...
  ├─ PowerShell demo scripts (scripts/demo-*.ps1)
...
| Azure SRE Agent | ~$0–10/month | Depends on incident volume |
| Log Analytics | ~$5–15/month | Minimal data volume |
| **Total (demo days only)** | **~$40–60/month** | Shut down ArcBox VMs when not demoing |
...
| 5–10 min | Architecture overview | 2-tier diagram: PowerShell Scripts + SRE Agent |
| 10–20 min | **Live: Automation** | Run `scripts/demo-a-health-check.ps1` → report; CMDB sync |
| 20–30 min | **Live: SRE Agent** | Alert storm → SRE Agent correlates → GLPI ticket. Stop Defender agent → SRE Agent diagnoses. |
| 30–35 min | **Live: Scripts + GLPI** | Show script output, GLPI tickets, CMDB records |
