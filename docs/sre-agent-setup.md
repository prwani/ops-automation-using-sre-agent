...
Verify: In the SRE Agent chat, type `list my resources` — you should see all 5 Arc servers.

## Step 3: Connect Azure Monitor Alerts (Incident Source)

1. In SRE Agent portal, go to **Integrations → Incident platforms**
2. Click **Azure Monitor**
3. Select the subscription and link the alert rules:
   - `alert-heartbeat-loss` (Severity 1)
   - `alert-high-cpu` (Severity 2)
   - `alert-low-disk` (Severity 2)
4. Click **Save**

Now when any alert fires, SRE Agent automatically receives it as an incident.
...
| Instructions | "Investigate the incident by checking server health, recent changes, and correlating with other alerts. Use the wintel-health-check-investigation and security-agent-troubleshooting skills. Propose remediation but wait for human approval before executing." |
...
| Instructions | "Investigate and auto-remediate if safe. For high CPU: identify top process. For low disk: identify largest files. For agent issues: attempt service restart. Create a GLPI ticket with findings." |

## Step 5: Upload Skills

For each skill in `sre-skills/`, upload via the Builder UI:

### 5a: Health Check Investigation Skill
...
6. Attach tools:
   ...
   - `query-perf-trends` (custom Kusto — create in Step 6 first)
7. Click **Save**

### 5b: Security Agent Troubleshooting Skill
...
### 5d: Compliance Investigation Skill
...
### 5e: VMware BAU Operations Skill
